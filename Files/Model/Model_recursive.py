...
class SISR(object):
    ...
    def psnr(self,target, ref):
        ...
        fin = tf.multiply(tf.constant(20.),self.log10(tf.div(tf.constant(1.),rmse)));#minus 20 to maximize
        return fin
    ...
    def build_model(self):
        ...
        self.weights = {
          'w1': tf.Variable(tf.random_normal([3, 3, 1, 24], stddev=1e-3), name='w1'),
          'w2': tf.Variable(tf.random_normal([3, 3, 6, 24], stddev=1e-3), name='w2'),
          'w3': tf.Variable(tf.random_normal([3, 3, 6, 24], stddev=1e-3), name='w3'),
          'w4': tf.Variable(tf.random_normal([3, 3, 6, 24], stddev=1e-3), name='w4'),
          'w5': tf.Variable(tf.random_normal([3, 3, 6, 24], stddev=1e-3), name='w5'),
          'w6': tf.Variable(tf.random_normal([3, 3, 6, 3], stddev=1e-3), name='w6')}
        self.biases = {
          'b1': tf.Variable(tf.zeros([24]), name='b1'),
          'b2': tf.Variable(tf.zeros([24]), name='b2'),
          'b3': tf.Variable(tf.zeros([24]), name='b3'),
          'b4': tf.Variable(tf.zeros([24]), name='b4'),
          'b5': tf.Variable(tf.zeros([24]), name='b5'),
    ...
    def model(self):
        conv1 = self.recursive(tf.nn.conv2d(self.images, self.weights['w1'], strides=[1,1,1,1], padding='SAME') + self.biases['b1'])
        conv2 = self.recursive(tf.nn.conv2d(conv1, self.weights['w2'], strides=[1,1,1,1], padding='SAME') + self.biases['b2'])
        conv3 = self.recursive(tf.nn.conv2d(conv2, self.weights['w3'], strides=[1,1,1,1], padding='SAME') + self.biases['b3'])
        conv4 = self.recursive(tf.nn.conv2d(conv3, self.weights['w4'], strides=[1,1,1,1], padding='SAME') + self.biases['b4'])
        conv5 = self.recursive(tf.nn.conv2d(conv4, self.weights['w5'], strides=[1,1,1,1], padding='SAME') + self.biases['b5'])
