...
class SISR(object):
    ...
    def __init__(self, sess, image_size=24,label_size=48, batch_size=2,c_dim=1, checkpoint_dir=None, sample_dir=None):
        self.sess = sess
    ...
    def build_model(self):
        ...
        self.weights = {
          ...
          'w6': tf.Variable(tf.random_normal([3, 3, 8, 3], stddev=1e-3), name='w6')}
        self.biases = {
    ...
    def model(self):
        conv1 = self.minout_unit(tf.nn.conv2d(self.images, self.weights['w1'], strides=[1,1,1,1], padding='SAME') + self.biases['b1'])
        conv2 = self.minout_unit(tf.nn.conv2d(conv1, self.weights['w2'], strides=[1,1,1,1], padding='SAME') + self.biases['b2'])
        conv3 = self.minout_unit(tf.nn.conv2d(conv2, self.weights['w3'], strides=[1,1,1,1], padding='SAME') + self.biases['b3'])
        conv4 = self.minout_unit(tf.nn.conv2d(conv3, self.weights['w4'], strides=[1,1,1,1], padding='SAME') + self.biases['b4'])
        conv5 = self.minout_unit(tf.nn.conv2d(conv4, self.weights['w5'], strides=[1,1,1,1], padding='SAME') + self.biases['b5'])
