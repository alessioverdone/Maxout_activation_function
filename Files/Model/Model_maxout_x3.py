...
class SISR(object):
    ...
    def __init__(self, sess, image_size=24,label_size=72, batch_size=1,c_dim=1, checkpoint_dir=None, sample_dir=None):
        self.sess = sess
    ...
    def Interpolationx3(self,y):
        """
        Final z-dim must be equal to hr dimension 48x48
        y = list of 3 images or one image 192x24
        HR dim = 72x72
        LR dim = 24x24
        r=2
        For each little square,one pixel from LR and three from subpixel image
        ...
        for i in range((y1.get_shape().as_list())[0]):
            t = tf.stack([x[i],y1[i],y2[i],y3[i],y4[i],y5[i],y6[i],y7[i],y8[i]],-1)
            t2 = tf.concat([t2,t],-1)
        z=tf.reshape(t2,[72,72])
    ...
    def build_model(self):
        ...
        self.weights = {
          'w1': tf.Variable(tf.random_normal([3, 3, 1, 16], stddev=1e-3), name='w1'),
          'w2': tf.Variable(tf.random_normal([3, 3, 8, 16], stddev=1e-3), name='w2'),
          'w3': tf.Variable(tf.random_normal([3, 3, 8, 16], stddev=1e-3), name='w3'),
          'w4': tf.Variable(tf.random_normal([3, 3, 8, 16], stddev=1e-3), name='w4'),
          'w5': tf.Variable(tf.random_normal([3, 3, 8, 16], stddev=1e-3), name='w5'),
          'w6': tf.Variable(tf.random_normal([3, 3, 8, 8], stddev=1e-3), name='w6')}
        self.biases = {
          'b1': tf.Variable(tf.zeros([16]), name='b1'),
          'b2': tf.Variable(tf.zeros([16]), name='b2'),
          'b3': tf.Variable(tf.zeros([16]), name='b3'),
          'b4': tf.Variable(tf.zeros([16]), name='b4'),
          'b5': tf.Variable(tf.zeros([16]), name='b5'),
    ...
    def model(self):
        conv1 = self.maxout(tf.nn.conv2d(self.images, self.weights['w1'], strides=[1,1,1,1], padding='SAME') + self.biases['b1'])
        conv2 = self.maxout(tf.nn.conv2d(conv1, self.weights['w2'], strides=[1,1,1,1], padding='SAME') + self.biases['b2'])
        conv3 = self.maxout(tf.nn.conv2d(conv2, self.weights['w3'], strides=[1,1,1,1], padding='SAME') + self.biases['b3'])
        conv4 = self.maxout(tf.nn.conv2d(conv3, self.weights['w4'], strides=[1,1,1,1], padding='SAME') + self.biases['b4'])
        conv5 = self.maxout(tf.nn.conv2d(conv4, self.weights['w5'], strides=[1,1,1,1], padding='SAME') + self.biases['b5'])
        conv6 = tf.nn.conv2d(conv5, self.weights['w6'], strides=[1,1,1,1], padding='SAME') + self.biases['b6']        
        interpolation= tf.squeeze(self.Interpolationx3(conv6))
